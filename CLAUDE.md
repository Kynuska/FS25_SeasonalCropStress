...
| `addModEventListener` with partial table (only `mouseEvent` defined) | FS25 calls `keyEvent()`, `update()`, `draw()`, and `delete()` on **every** registered listener **without nil-checking** the method first. A table missing `keyEvent` causes a silent Lua error the instant any key is pressed (including ESC). FS25's pcall catches it, marks the key as consumed, and InGameMenu never opens — ESC appears completely broken. | Always define ALL listener methods. Stubs are fine: `update=function()end, draw=function()end, delete=function()end, keyEvent=function()end, mouseEvent=function(self,...)...end`. **Confirmed in-game: partial table = ESC broken.** |
